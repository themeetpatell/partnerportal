...
import { sendTeamMemberPasswordResetEmail } from "@repo/notifications"
...
export async function POST(req: NextRequest) {
  ...
  const limited = rateLimit(`admin-users:reset:${userId}`, 10, 60_000)
  ...
  if (linkError) {
    console.error("[POST /api/admin/users/reset-password] Error:", linkError)
