...
CREATE INDEX IF NOT EXISTS lead_quotes_tenant_lead_idx ON public.lead_quotes (tenant_id, lead_id);

CREATE UNIQUE INDEX IF NOT EXISTS lead_quotes_tenant_engine_quote_uidx
  ON public.lead_quotes (tenant_id, engine_quote_id)
  WHERE deleted_at IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS lead_quotes_idempotency_uidx
  ON public.lead_quotes (tenant_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL AND deleted_at IS NULL;

CREATE TABLE IF NOT EXISTS public.pricing_engine_webhook_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
  tenant_id uuid NOT NULL REFERENCES public.tenants (id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS pricing_engine_webhook_events_tenant_created_idx
  ON public.pricing_engine_webhook_events (tenant_id, created_at);
