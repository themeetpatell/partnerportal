...
function ViewValue({ field, value }: { field: LeadFieldDef; value: FieldValue }) {
  ...
  if (field.kind === "readonlyDate") {
    return <p className="text-sm text-foreground">{formatReadonlyDate(value)}</p>
  }
    ...
    return (
      <div className="flex flex-wrap gap-1.5">
        {list.map((s) => (
          <span
            key={s}
            className="inline-flex items-center gap-1 rounded-lg border border-border bg-secondary/70 px-2.5 py-1 text-xs text-[var(--portal-text-soft)]"
          >
  ...
  if (field.kind === "number") {
    return <p className="text-sm text-foreground">{formatNumberDisplay(field.name, value)}</p>
  }
...
const inputClass = "field-input h-10"
const textareaClass = "field-input h-auto py-2"
          ...
          isEditing ? (
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-secondary px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary/80 hover:text-foreground disabled:opacity-50"
              >
              ...
              <button
                ...
                onClick={handleSave}
                disabled={showSpinner}
                className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50"
              >
                {showSpinner ? (
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 self-start rounded-lg bg-secondary/60 px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground"
            >
