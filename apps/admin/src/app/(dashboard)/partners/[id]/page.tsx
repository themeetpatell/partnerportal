...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium border capitalize ${map[status] ?? "bg-zinc-800 border-zinc-700 text-zinc-400"}`}
    >
      {status.replace("_", " ")}
    </span>
  )
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {partner.status === "approved" && (
          <form action={`/api/partners/${partner.id}/reject`} method="POST">
            <button
              type="submit"
              className="inline-flex items-center gap-2 bg-zinc-700 hover:bg-zinc-600 text-zinc-200 px-4 py-2 rounded-lg text-sm font-medium transition-colors"
            >
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <dd className="text-zinc-200 text-sm">{partner.companyName}</dd>
              </div>
              <div>
                <dt className="text-zinc-500 text-xs font-medium uppercase tracking-wider mb-1">
                  Partner Type
                </dt>
                <dd className="text-zinc-200 text-sm capitalize">
                  {partner.type}
                </dd>
              </div>
              <div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl">
            ...
            {partnerLeads.length === 0 ? (
              <div className="px-6 py-8 text-center">
                <p className="text-zinc-500 text-sm">No leads submitted yet</p>
              </div>
            ) : (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            ...
            <div className="text-center py-4">
              ...
              <p className="text-zinc-500 text-xs mt-1">Total all-time</p>
            </div>
            <Link
              ...
              <div className="space-y-2">
                {partnerDocs.map((doc) => (
                  ...
                  >
                    ...
                    <ExternalLink className="w-3.5 h-3.5 text-zinc-600 group-hover:text-zinc-400 transition-colors flex-shrink-0" />
                  </a>
                ))}
              </div>
            )}
